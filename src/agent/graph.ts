...
export const GRAPH_STRUCTURE =
  "START -> discoverDirectJobs -> (zero direct: buildSummary | loadProfile -> extractJobDetails -> matchJobs -> rankJobs -> selectReadyToApply -> buildSummary) -> END";

export function initialJobAgentState(): JobAgentState {
  return { profile: undefined, jobs: [], detailedJobs: [], directJobs: [], manualJobs: [], discovery: undefined, matches: [], rankedMatches: [], readyToApplyJobs: [], selectedApplication: undefined, applicationResult: undefined, summary: undefined, errors: [] };
}
...
export function createJobAgentGraph(dependencies: JobAgentDependencies) {
  return new StateGraph(JobAgentStateAnnotation)
    ...
    .addConditionalEdges(
      "discoverDirectJobs",
      (state) => state.directJobs.length === 0 ? "buildSummary" : "loadProfile",
